...
AI is provided only by ai.teacher (local Qwen3-4B); no external AI API.
"""
...
def init_db():
    with db() as c:
        c.executescript("""
        CREATE TABLE IF NOT EXISTS videos(video_id TEXT PRIMARY KEY,title TEXT NOT NULL,url TEXT NOT NULL,thumbnail TEXT,duration INTEGER DEFAULT 0,status TEXT DEFAULT 'new',last_sentence INTEGER DEFAULT 0,updated_at TEXT DEFAULT CURRENT_TIMESTAMP);
        CREATE TABLE IF NOT EXISTS sentences(video_id TEXT NOT NULL,sentence_index INTEGER NOT NULL,text TEXT NOT NULL,start REAL NOT NULL,end_time REAL NOT NULL,PRIMARY KEY(video_id,sentence_index));
        CREATE TABLE IF NOT EXISTS practice(id INTEGER PRIMARY KEY AUTOINCREMENT,video_id TEXT,sentence_index INTEGER,score REAL,similarity REAL,completeness REAL,fluency REAL,wpm REAL,recognized TEXT,created_at TEXT DEFAULT CURRENT_TIMESTAMP);
        """)
init_db()

def clean(text): return re.sub(r"\s+", " ", (text or "").replace("\n", " ")).strip()
def stamp(seconds):
    n=max(0,int(seconds or 0)); return f"{n//60:02d}:{n%60:02d}"
...
def translate(sentence):
    ...
    r=AITeacher().analyze(sentence); return f"### 🇻🇳 Nghĩa tự nhiên\n{r.translation or 'Chưa có.'}"
