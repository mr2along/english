...
import os
import gradio as gr
from app import APP_NAME, DEFAULT_PLAYLIST, load_library, load_lesson, select_sentence, translate, check_speaking, stats
...
def grade_quiz(choice, answer, q):
    ...
    letter=choice.split(".",1)[0].strip().upper(); ok=letter==answer.strip().upper()
...
CSS=""".gradio-container{max-width:1180px!important}.hero{padding:22px;border-radius:22px;margin-bottom:14px;background:linear-gradient(135deg,#0f172a,#334155);color:white}.hero h1{margin:0;font-size:30px}.hero p{margin:5px 0 0;opacity:.82}.step{border:1px solid #e2e8f0;border-radius:18px;padding:18px;margin:8px 0}.sentence-main{font-size:26px;line-height:1.45;font-weight:600;padding:22px;border-radius:18px;border:1px solid #cbd5e1;min-height:100px}@media(max-width:700px){.gradio-container{padding:8px!important}.hero h1{font-size:24px}.sentence-main{font-size:21px}.step{padding:12px}}"""


def main():
    with gr.Blocks(title=f"{APP_NAME} V2.5",css=CSS,theme=gr.themes.Soft()) as ui:
        ...
        with gr.Tab("📚 Library"):
            ...
            import_btn.click(load_library,playlist,[video,library,status,dashboard]); video.change(load_lesson,[video,library],[player,sentences,transcript_status,title])
        with gr.Tab("🎯 Learning Session"):
            ...
            current_sentence=gr.Markdown("<div class='sentence-main'>Chọn video để bắt đầu.</div>"); current_text=gr.State(""); timestamp=gr.Markdown()
            ...
            with gr.Group(elem_classes="step"):
                gr.Markdown("### 4️⃣ AI Teacher"); teacher_btn=gr.Button("🧑‍🏫 Phân tích câu",variant="primary"); teacher=gr.Markdown(); teacher_progress=gr.Markdown()
            with gr.Group(elem_classes="step"):
        ...
        def current(i,ss):
            ...
            return f"<div class='sentence-main'>{text or 'Chưa có câu.'}</div>",text,f"⏱️ {stamp}",session_status(idx,ss),idx
        sentence_index.change(current,[sentence_index,sentences],[current_sentence,current_text,timestamp,session_progress,sentence_index])
