...
MAP={v['video_id']:v for v in VIDEOS}; TOTAL=sum(len(v.get('transcript',[])) for v in VIDEOS)
...
with gr.Blocks(title='English Learning Lab V2.4',css=CSS,js=JS,theme=gr.themes.Soft()) as demo:
    gr.HTML(f"<div class='hero'><h1>🎧 English Learning Lab</h1><div class='sub'>Listening · Reading · Shadowing · Pronunciation · Grammar · Vocabulary · Quiz · Spaced Repetition</div><span class='badge'>V2.4 · {len(VIDEOS)} lessons · {TOTAL:,} transcript segments</span></div>")
    with gr.Row(elem_classes='stats'):
        gr.HTML(f"<div class='stat'><div class='num'>{len(VIDEOS):,}</div><div class='label'>📚 Bài học</div></div>"); gr.HTML(f"<div class='stat'><div class='num'>{TOTAL:,}</div><div class='label'>📝 Câu transcript</div></div>"); gr.HTML(f"<div class='stat'><div class='num'>{len(PROGRESS.data['lessons']):,}</div><div class='label'>📈 Đã học</div></div>")
