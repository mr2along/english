...
def _caption_sentences(video_id):
    """Fetch English captions through Invidious only; never call YouTube/yt-dlp here."""
    errors = []
    for base in INVIDIOUS_INSTANCES:
        try:
            r = requests.get(f"{base}/api/v1/captions/{video_id}", params={"label": "English"}, headers={"User-Agent": "EnglishLearningLab/3.0", "Accept": "application/json"}, timeout=(5, 12))
            r.raise_for_status()
            captions = r.json().get("captions") or []
            ...
            if not cap:
                ...
                continue
            cr = requests.get(cap["url"], headers={"User-Agent": "EnglishLearningLab/3.0"}, timeout=(5, 15))
            cr.raise_for_status(); text = cr.text; pieces = []
            if text.lstrip().startswith("{"):
                for ev in cr.json().get("events") or []:
                    txt = re.sub(r"\s+", " ", " ".join(str(seg.get("utf8", "")) for seg in (ev.get("segs") or []))).strip()
                    if txt:
                        s=float(ev.get("tStartMs",0))/1000.0; e=s+float(ev.get("dDurationMs",0))/1000.0; pieces.append((txt,s,e))
            else:
                for m in re.finditer(r'<text[^>]*start="([0-9.]+)"[^>]*dur="([0-9.]+)"[^>]*>(.*?)</text>', text, re.S):
                    txt=re.sub(r"\s+"," ",html.unescape(re.sub(r"<[^>]+>"," ",m.group(3)))).strip()
                    if txt:
                        s=float(m.group(1));pieces.append((txt,s,s+float(m.group(2))))
            if not pieces:
                errors.append(f"{base}: empty caption response"); continue
            result=[];buf=[];start=end=None
            for txt,s,e in pieces:
                start=s if start is None else start;end=e;buf.append(txt);sentence=re.sub(r"\s+"," "," ".join(buf)).strip()
                if re.search(r"[.!?…]$",sentence) or len(sentence)>=180:
                    result.append({"index":len(result),"text":sentence,"start":start,"end":end});buf=[];start=end=None
            if buf:result.append({"index":len(result),"text":re.sub(r"\s+"," "," ".join(buf)).strip(),"start":start or 0,"end":end or 0})
            return result,f"✅ {len(result)} câu · nguồn: Invidious {base}"
        except Exception as e:
            errors.append(f"{base}: {type(e).__name__}: {e}")
    return [],"❌ Invidious không cung cấp English captions. " + "; ".join(errors)

def load_lesson(choice, items):
    """Load a lesson without any yt-dlp fallback."""
    video = _selected_video(choice, items)
    if not video:
        return "", [], "❌ Chưa chọn video.", ""
    vid=str(video.get("id", "")); sentences,status=_caption_sentences(vid)
    ...
    return embed,sentences,status,f"### {video.get('title',vid)}"
...
def import_library(url):
    video_update,items,status,dash=load_library(url);choices=video_update.get("choices",[]);value=video_update.get("value");return video_update,gr.update(choices=choices,value=value),items,status,dash
def render_sentence_zero(ss):
    if not ss:return "<div class='sentence-main'>Chưa có transcript cho video này.</div>","","",session_status(0,ss),0
    text,raw,timestamp,idx=select_sentence(0,ss);return text,raw,timestamp,session_status(0,ss),idx
def render_sentence(i,ss):
    if not ss:return "<div class='sentence-main'>Chưa có transcript cho video này.</div>","","",session_status(0,ss),0
    text,raw,timestamp,idx=select_sentence(i,ss);return text,raw,timestamp,session_status(i,ss),idx
def open_session(choice,items):
    if not choice or not items:return "","[]","❌ Chưa chọn video.","","<div class='sentence-main'>Chọn video để bắt đầu.</div>","","", "Chưa có bài học.",0
    player,ss,status,title=load_lesson(choice,items)
    if ss:
        text,raw,timestamp,idx=select_sentence(0,ss);progress=session_status(0,ss)
    else:
...
def main():
    with gr.Blocks(title=f"{APP_NAME} V2.7",css=CSS,theme=gr.themes.Soft()) as ui:
        ...
        with gr.Tab("📚 Library"):
            with gr.Row():playlist=gr.Textbox(label="YouTube Playlist",value=DEFAULT_PLAYLIST,scale=8);import_btn=gr.Button("📥 Import",variant="primary",scale=2)
        ...
        with gr.Tab("🎯 Learning Session"):
            session_video=gr.Dropdown(label="🎬 Chọn video để luyện",choices=[],interactive=True);session_progress=gr.Markdown("Chưa có bài học.");sentence_index=gr.Number(value=0,visible=False,elem_id="sentence_index")
