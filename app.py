...
def next_index(idx, ss):
    return min(len(ss) - 1, int(idx or 0) + 1) if ss else 0


def prev_index(idx, ss):
    return max(0, int(idx or 0) - 1) if ss else 0


def session_status(idx, ss):
    if not ss:
        return "Chưa có bài học."
    i = max(0, min(int(idx or 0), len(ss) - 1))
    return f"### Câu {i+1} / {len(ss)} · {round((i+1)/len(ss)*100)}%"
    ...
    with gr.Blocks(title=f"{APP_NAME} V2.7", css=CSS, theme=gr.themes.Soft()) as ui:
        ...
        with gr.Tab("🎯 Learning Session"):
            ...
            with gr.Group(elem_classes="step"):
                ...
                hide_transcript = gr.Checkbox(label="🔒 Ẩn transcript", value=True)
                reveal = gr.Button("👁 Hiện câu")
                transcript_reveal = gr.Markdown("Transcript đang ẩn.")

            with gr.Group(elem_classes="step"):
                gr.Markdown("### 2️⃣ Translate")
            ...
            with gr.Group(elem_classes="step"):
                gr.Markdown("### 4️⃣ AI Teacher — Local Qwen (auto hardware)")
        ...
        sentence_index.change(
            ...
            [current_sentence, current_text, timestamp, session_progress, sentence_index],
        )
        ...
        reveal.click(
            lambda hidden, text: text if not hidden else "🔒 Transcript đang ẩn. Hãy nghe và tự nhớ câu trước.",
            [hide_transcript, current_text], transcript_reveal,
        )
        hide_transcript.change(
            lambda hidden: "🔒 Transcript đang ẩn." if hidden else "👁 Transcript đã bật.",
            hide_transcript, transcript_reveal,
        )
        translate_btn.click(translate, current_text, translation)
        speak_btn.click(check_speaking, [current_text, audio, sentence_index], [speaking, recognized])
        teacher_btn.click(analyze_teacher, [current_text, sentence_index], [teacher, quiz_state, teacher_progress]).then(
            quiz_options, quiz_state, [quiz_question, quiz_choice, quiz_answer]
        )
        quiz_btn.click(grade_quiz, [quiz_choice, quiz_answer, quiz_state], quiz_result)
        complete.click(next_index, [sentence_index, sentences], sentence_index).then(
            lambda: stats() + " · " + learning_stats(), outputs=dashboard
        )
