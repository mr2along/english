...
def video_id(value: str) -> str | None:
    ...
    m = re.search(r"(?:v=|youtu\.be/|youtube\.com/shorts/|youtube\.com/embed/)([A-Za-z0-9_-]{11})", value)
    if m:
        return m.group(1)
...
def youtube_embed(vid: str | None) -> str:
    ...
    return ("<div class='yt-wrap'><iframe id='english-youtube-player' "
...
def get_ytt() -> YouTubeTranscriptApi:
    username = _secret("WEBSHARE_PROXY_USERNAME", "WEBSHARE_USERNAME")
    password = _secret("WEBSHARE_PROXY_PASSWORD", "WEBSHARE_PASSWORD")
    if not username or not password:
        raise RuntimeError("Thiếu Webshare credentials trong HF Secrets.")
    from youtube_transcript_api.proxies import WebshareProxyConfig
    proxy_config = WebshareProxyConfig(proxy_username=username, proxy_password=password, retries_when_blocked=10)
...
def load_video(url: str):
    ...
    first = sentences[0]
    title = f"### 🎬 Video `{vid}`\n\n✅ **youtube-transcript-api + WebshareProxyConfig** · language: **{language}** · **{len(sentences)} câu**"
...
() => {
  const seekToSentence = (seconds) => {
    ...
    const message = JSON.stringify({event:'command',func:'seekTo',args:[Number(seconds),true]});
...
SEEK_JS = """
(seconds) => {
  if (window.__englishLabSeek) window.__englishLabSeek(Number(seconds));
  return [];
}
"""
...
with gr.Blocks(title=APP_NAME, theme=gr.themes.Soft(), css=CSS, js=YOUTUBE_JS) as demo:
    ...
    import_btn.click(load_video, inputs=url, outputs=[lesson_title,state,sentence,hidden,translation,progress,seek_time,status,video_frame])
