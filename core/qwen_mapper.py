...
The raw YouTube transcript is preserved. Qwen only decides where sentence
boundaries belong; timestamps are then calculated from the original segment
boundaries, so the model cannot invent timings.
"""
import json
import os
import re
...
def map_segments(raw: List[Dict]) -> List[Dict]:
    """Map raw fragments to natural sentences and synchronize timestamps."""
    if not raw: return []
    try:
...
def map_video(video: Dict) -> Dict:
    """Return a copy with raw_transcript + Qwen-mapped transcript."""
    raw=list(video.get("transcript") or [])
    mapped=map_segments(raw)
    result=dict(video)
    result["raw_transcript"]=raw
    result["transcript"]=mapped
    result["transcript_mapping"]={"engine":"qwen3","model":os.getenv("QWEN_MODEL","Qwen/Qwen3-0.6B"),"raw_segments":len(raw),"sentences":len(mapped)}
    return result
